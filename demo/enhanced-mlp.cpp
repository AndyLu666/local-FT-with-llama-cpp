...
public:
    ...
    static std::vector<std::vector<float>> forward(
        ...
        std::cout << "執行前向傳播，輸入 batch 大小: " << input[0].size() << std::endl;
...
public:
    ...
    void zero_grad() {
        ...
        // 在這個實作中，梯度每次都重新計算，所以不需要顯式清空
    }

private:
    float learning_rate_;
    const float max_grad_ = 1.0f; // 梯度裁剪閾值
...
public:
    ...
    void save_model(const std::string& filepath) {
        ...
        // 保存第一层偏置
        for (float val : bias1_) {
            file.write(reinterpret_cast<const char*>(&val), sizeof(val));
        }
        
        // 保存第二层权重
        for (const auto& row : weights2_) {
            for (float val : row) {
                file.write(reinterpret_cast<const char*>(&val), sizeof(val));
            }
        }
        ...
        // 保存第二层偏置
        for (float val : bias2_) {
            file.write(reinterpret_cast<const char*>(&val), sizeof(val));
        }
        
        file.close();
        std::cout << "模型已保存到: " << filepath << std::endl;
    }
    ...
    bool load_model(const std::string& filepath) {
        ...
        file.read(reinterpret_cast<char*>(&loaded_input_size), sizeof(loaded_input_size));
        file.read(reinterpret_cast<char*>(&loaded_hidden_size), sizeof(loaded_hidden_size));
        file.read(reinterpret_cast<char*>(&loaded_output_size), sizeof(loaded_output_size));
        
        // 检查网络结构是否匹配
        if (loaded_input_size != input_size_ || loaded_hidden_size != hidden_size_ || loaded_output_size != output_size_) {
            std::cerr << "模型结构不匹配!" << std::endl;
            std::cerr << "期望: " << input_size_ << "x" << hidden_size_ << "x" << output_size_ << std::endl;
            std::cerr << "文件: " << loaded_input_size << "x" << loaded_hidden_size << "x" << loaded_output_size << std::endl;
            return false;
        }
        ...
        file.close();
        std::cout << "模型已从文件加载: " << filepath << std::endl;
        return true;
    }
...
int main() {
    ...
    // 訓練
    std::cout << "開始訓練增強版多層感知器...\n";
    std::cout << "使用您的 forward、backward 和 optimizer 組件\n";
    ...
    std::cout << "========================================\n";
    std::cout << "訓練完成！使用了您的組件架構\n";
    
    // 保存训练好的模型
    std::string model_path = "enhanced_mlp_model.bin";
    model.save_model(model_path);
    
    // 测试模型加载
    std::cout << "\n测试模型加载...\n";
    EnhancedMLP test_model(n_features, n_hidden, n_classes, learning_rate);
